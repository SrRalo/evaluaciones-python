...
class reporte12(object):
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "Reporte Evaluaciones a Docente"))
        self.label.setText(_translate("Dialog", "Generar Reporte de Evaluaciones a Docentes:"))
    ...
    def visualizar_datos(self):
        ...
        self.results = join_tables(
            table1="evaluacionesTI.Detalle_EvaDocente",
            table2="evaluacionesTI.Evaluacion_Asignada",
            common_column="id_evaluacion_asignada",
            columns_to_select=["detalle_evadocente.*", "evaluacion_asignada.nombre_evaluacion"],
            conditions=f"detalle_evadocente.id_evaluacion_asignada = {id_evaluacion_asignada}"
        )
        ...
        if self.results:
            ...
            for row in self.results:
                item_text = (
                    f"ID Evaluación Docente: {row['id_evadocente']}\n"
                    f"Nota: {row['nota_evadocente']}\n"
                    f"ID Evaluación Asignada: {row['id_evaluacion_asignada']}\n"
                    f"Nombre Evaluación: {row['nombre_evaluacion']}\n\n"
                )
                self.listWidget.addItem(item_text)  # Agregar cada item formateado a la lista
        else:
    ...
    def generar_pdf(self):
        ...
        datos = [["ID Docente", "Nota", "ID Evaluación", "Nombre Evaluación"]]
